...
class MangaDexApi {
	...
	getReadableMangaChapters = async (mangaId, limit = 500) => {
		try {
			const response = await fetch(
				`${this.BaseManga}/${mangaId}/feed?limit=${limit}&includeExternalUrl=0&order[volume]=asc&order[chapter]=asc&contentRating[]=safe&contentRating[]=suggestive&contentRating[]=erotica&contentRating[]=pornographic`,
				{ headers: { ...this.Headers } }
			).then((data) => data.json());

			return response?.data?.reduce((volumes, chapter) => {
				if (!chapter?.attributes?.pages) return volumes;

				const volumeKey = chapter?.attributes?.volume || 'none';
				const chapterKey = chapter?.attributes?.chapter || 'none';
				const previousChapter = volumes?.[volumeKey]?.chapters?.[chapterKey];

				return {
					...volumes,
					[volumeKey]: {
						volume: volumeKey,
						count: (volumes?.[volumeKey]?.count || 0) + 1,
						chapters: {
							...(volumes?.[volumeKey]?.chapters || {}),
							[chapterKey]: previousChapter
								? {
										...previousChapter,
										others: [...(previousChapter.others || []), chapter.id],
										count: (previousChapter.count || 1) + 1,
								  }
								: {
										chapter: chapterKey,
										id: chapter.id,
		...
		try {
			...
			const originalLanguages =
				originalLanguage.length > 0
					? originalLanguage?.map((el) => `&originalLanguage[]=${el}`).join('')
					: '';
			...
			return await fetch(
				`${
					this.BaseManga
				}?limit=${limit}&offset=${offset}&includes[]=cover_art&includes[]=author&includes[]=artist${contentRating}${statusElems}${pubDemographics}${includeTags}${excludeTags}${originalLanguages}${
