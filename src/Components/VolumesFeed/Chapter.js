...
const ChapterEl = ({ item, index, chapter, chName }) => {
    ...
    const { id: urlMangaId } = useParams();
    const mangaId = filterSomeAttribute(item?.relationships, 'manga')?.id || urlMangaId;
    ...
    const handleRead = () => {
        const volume = item?.attributes?.volume || 'none';
        const chapter = item?.attributes?.chapter || 'none';
        if (mangaId) {
            navigate(`/chapter/${mangaId}/${volume}/${chapter}`);
        }
    }
    ...
    return (
        <div className={styles.content}>
            ...
            <div className={styles.chapter_name} onClick={handleRead}>
