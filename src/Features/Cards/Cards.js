    ...
    return (
        <div className={styles.wrapp} style={{paddingBottom: '48px'}}>
            ...
            <div ref={setRefContent} className={styles.content}>
                {loading 
                    ? <Spinner customStyle={{width: 40, height: 40, borderColor: '#ff6740'}} /> 
                    : mangaArrayInfo.length > 0 && mangasArr.length > 0
                        ...
                        : <div className={styles.couldnt_find} style={{width: '100%', padding: '24px 0', backgroundColor: '#f6f7fa', color: '#68707a', borderRadius: '0.5rem', fontSize: '13pt', fontWeight: '500', textAlign: 'center'}}>No Data Found</div>
                }
            </div>
        </div>
    );
...
const ChooseCardViewType = ({ handleManga, type, manga, mangaInfo, mangaStatistics, setRefCover, setRefTitle, setRefDescription }) => {
    switch(type) {
        ...
        case 'blocks': {
            const coverFileName = filterSomeAttribute(mangaInfo?.relationships, 'cover_art', 'fileName');
            const coverUrl = coverFileName !== 'No fileName'
                ? `https://uploads.mangadex.org/covers/${mangaInfo?.id}/${coverFileName}`
                : 'https://placehold.co/400x600?text=No+Cover';
            const blocksTitle = mangaInfo?.attributes?.title?.en || mangaInfo?.attributes?.title?.['ja-ro'] || mangaInfo?.attributes?.title?.ko || mangaInfo?.attributes?.title?.zh || (mangaInfo?.attributes?.title ? Object.values(mangaInfo.attributes.title)[0] : 'Untitled');
            return (
                <Cover
                    src={coverUrl}
                    alt={blocksTitle}
